...
function enable_bulk_edit(none_selected_message, size)
{
	...
	if($("#sortable_table tbody :checkbox:checked").length >0)
	{
	    ...
	    tb_show($(this).attr('title'),$(this).attr('href')+"/"+row_ids[0]+size,false);
	    $(this).blur();
	}
	else
	{
...
function enable_row_selection(rows)
{
	...
	var resp = !checkbox.prop('checked');
